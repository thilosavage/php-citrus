...
class render {
	function view($view, $data = array()) {
		...
		$g = explode('/',$view, 2);
		if ($g[0] == 'common') {
			include(site::root.'common_views/'.$g[1].'.php');
		}
		else {
	...
	function js() {
		...
		$handler = opendir(site::root.'common_js');
		...
		foreach ($files as $fileName){
			$path = site::root.'common_js/'.$fileName;
			ob_start();
			include($path);
			$output = ob_get_clean();
			echo $output;
	...
	function css() {
		...
		$handler = opendir(site::root.'common_css');
		while ($file = readdir($handler)) {
			if ($file != '.' && $file != '..' && strpos($file,'.'))
				$files[] = $file;
		}
		closedir($handler);
		foreach ($files as $fileName){
			$path = site::root.'common_css/'.$fileName;
			ob_start();
			include($path);
			$output = ob_get_clean(); 	
			echo $output;
